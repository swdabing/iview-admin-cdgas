...
export default {
  ...
  methods: {
    ...
    init () {
      if (this.$store.state.user.userName === '廖玮' || this.$store.state.user.userName === '陈海燕' ||
        ...
        this.$refs.tables.setSql('WITH SQL1 as (SELECT a.KPI_USER,a.KPI_GROUP,' +
                              'avg(a.TDXZ+a.GZGFX+a.LDJL) as AVG,' +
                              '0.05 as WEIGHT,' +
                              'avg(a.TDXZ+a.GZGFX+a.LDJL)*0.05 as SCORE,' +
                              '(SELECT (b.TDXZ+b.GZGFX+b.LDJL)*0.05 from kpi b where b.SCORER=\'黄远三\' and a.KPI_USER=b.KPI_USER) as HYS,' +
                              '(SELECT (c.TDXZ+c.GZGFX+c.LDJL)*0.1 from kpi c where c.SCORER=\'李涛\' and a.KPI_USER=c.KPI_USER) as LT,' +
                              ...
                              'SELECT SQL1.*,SQL1.SCORE+SQL1.HYS+SQL1.LT as TOTAL_SCORE FROM SQL1', '', 'local')
